...
function search() {
    ...
    for (let i = 1; i < tr.length; i++) {
        const td = tr[i].getElementsByTagName('td')[1]; // Kolom Anggota
        if (td) {
...
// Tambahkan event listener untuk input search agar jika dikosongkan secara manual, tabel kembali ke kondisi semula
document.addEventListener('DOMContentLoaded', function () {
    const input = document.getElementById('searchInput');
    input.addEventListener('input', function () {
        if (this.value === '') {
